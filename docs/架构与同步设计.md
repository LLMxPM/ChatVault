...
1. **目录 mtime 剪枝**：子目录 mtime 不晚于检查点时整棵子树跳过（适合微信 `msg/file/YYYY-MM` 这类“新文件写入会更新所在目录”的布局）。
2. **已知文件复检**：对 `local_files` 中路径做磁盘 re-stat；mtime/size/缓存缺失变化的重新入库（覆盖“仅内容变更、父目录 mtime 不变”的场景）。

编排层先用 `path_is_current` 跳过未变更文件，仅对候选变更文件做 50ms 稳定性检测，避免对全量文件串行 sleep。

已知限制：深层通用目录中，若新文件出现在“祖先目录 mtime 未更新”的子树里，增量剪枝可能漏检；此时应执行一次全量扫描校准。微信附件目录布局通常不受影响。
