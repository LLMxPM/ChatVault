...
媒体扩展已实施：视频 mp4 与聊天图片备份见[微信 4.x 媒体备份完整方案](compose/spec/wechat-4x-media-backup.md)。视频按独立媒体根（`msg/video`）增量扫描；图片在入库前增加本机离线解密和完整图片校验，将密文来源与明文对象分开，强制暂存待归档明文；微信密钥和派生材料不持久化、不上传，流程不访问微信进程内存。解密失败进入可重试的本机候选状态（`image_candidates` 表），不阻塞其他媒体根。
...
packages/
  ...
  ui/                  桌面界面组件
docs/
  compose/spec/        功能规格与实施设计
```

模块随实施阶段建立。SourceAdapter 封装微信版本对应的目录发现和元数据提取规则（媒体根、`conv_hash`、图片变体与解密也落在适配器内）；Scanner 负责统一扫描流程，增量发现采用手动触发与操作系统定时任务，不使用文件系统监听。WebDAV 模块封装连接、路径、能力探测与传输重试。Rust 核心通过明确接口供桌面壳和 CLI 调用。
...
3. **微信 4.x 图片**：扫描根为 `msg/attach`。发现阶段按检查点预筛：`Img` 目录 mtime 与源文件 mtime 均不晚于检查点时跳过；原地改写、缓存丢失由 `list_changed_known_files` 复检后重新 upsert（digest 比对决定是否重置候选）。解密失败与等待稳定的候选由 `image_candidates` 队列重试，不依赖目录 mtime。检查点在候选 upsert 完成后推进，代表「已发现」而非「全部备份成功」。
4. **全量扫描**：忽略检查点，所有来源均完整遍历。
...
| **图片候选** | `image_candidates` | 微信图片解密准备流水线中的源文件 |
| **系统计划任务** | schtasks `ChatVaultScheduledScan` | OS 层拉起 CLI，不是业务对象 |
...
成功文件不落明细。状态：`failed`（可重试）| `missing`（来源丢失/哈希不匹配）| `decrypt_failed` | `skipped`。明细可带 `task_id` 指向归档单元，用于重新入队。
...
见[微信 4.x 媒体备份](compose/spec/wechat-4x-media-backup.md)。摘要：`discovered → waiting_stable → preparing → decrypted_pending_verify → verified_plaintext → ingested → archived`，失败为 `failed`。仅 `ingested` 后生成归档单元。
...
位置状态表示 LOCAL_ONLY、REMOTE_ONLY、BOTH，与任务进度分别展示。源文件消失时更新本地可用性，远端归档独立保留；已入暂存的任务继续处理。文件库列表按内容对象折叠展示，展开可查看全部来源；每行展示本地/远程/本地+远程/失效徽章，可对本机路径直接打开或定位，对仅远程对象下载到 `download_dir`。
