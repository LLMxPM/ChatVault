...
一次「立即归档 / 定时归档」对应一条 `task_runs` 记录，下挂阶段（`task_run_stages`：scan → archive → publish → pull）与关键文件明细（`task_run_items`：失败、缺失、解密失败等；成功文件只进阶段计数）。

- 运行状态：`running | success | partial | failed`；有失败明细但仍跑完为 partial。
- 桌面流水线通过 Tauri 事件（`run://started|stage|progress|item|finished`）推送实时阶段与文件进度；CLI `scheduled-run` 同样写入运行记录。
- 启动时将残留 `running` 标记为失败；保留最近 50 次或 30 天运行。
- `upload_tasks` 仍是跨运行上传队列；明细中的 `task_id` 可重新入队。

切换 Vault 前必须先在设置页执行「清空旧 Vault 绑定」：解除 `remote_binding`，清除同步游标、本机日志、已应用事件与设备注册，并将归档任务重新排队。本地文件索引与来源映射保留。操作需输入当前完整 Vault ID 强制确认。
