...
媒体扩展的待实施设计见[微信 4.x 媒体备份完整方案](compose/spec/wechat-4x-media-backup.md)（含视频 mp4 与聊天图片）。视频按独立媒体根增量扫描；图片在入库前增加本机离线解密和完整图片校验，将密文来源与明文对象分开，强制暂存待归档明文；微信密钥和派生材料不持久化、不上传，流程不访问微信进程内存。解密失败进入可重试的本机候选状态，不阻塞其他媒体根。本文以下仍描述现有基线；媒体的上传来源、缓存回收、打开位置及元数据调整以完整方案为实施依据，通过验收后再更新基线。
