...
1. **发现**：在账号的图片根建立独立候选记录，并解析 `conv_hash` 与 `YYYY-MM`。默认增量：以图片根检查点预筛——`Img` 目录 mtime 与源文件 mtime 均不晚于检查点时跳过；`None` 检查点或强制全量时完整枚举。**不复用** `msg/file` 的月份 mtime 裁剪（深层 `Img` 新增未必改祖先 mtime），原地改写与缓存缺失由已知文件复检（`local_files` re-stat）后重新 upsert，digest 比对决定是否重置候选。候选上记录规范化 stem 与预分组键。
...
目录枚举与候选 upsert 完成后，推进图片根的扫描检查点（写入本轮开始时刻）。检查点代表「已发现」，不代表「全部成功备份」；解密/入库失败不阻塞检查点。未成功候选的重试独立于目录 mtime，由 `image_candidates` 的 `status`/`next_retry_ms` 驱动，每轮从 DB 队列重建待准备集合，不要求源文件在本轮被再次发现。

普通扫描以检查点预筛 + 源大小/mtime/digest 减少重复解密；显式全量复核须重新读取图片源摘要，以覆盖时间戳和大小均未改变的覆盖写。已完成归档且缓存正常回收的文件不因 `cache_path` 为空就重复解密或追加来源记录。拷贝进入的历史文件若 mtime 早于检查点且从未入库，增量无法发现，需显式全量。
...
3. **检查点只表示「已发现」**：目录枚举与候选 upsert 完成后推进图片根检查点；解密/入库失败不回退检查点，重试走候选队列。
4. **源摘要用于全量复核**：增量以检查点预筛 + size/mtime/digest 跳过；显式全量复核须重读源摘要。mtime 早于检查点且从未入库的历史拷贝需全量才能发现。
