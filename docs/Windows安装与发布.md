...
%LOCALAPPDATA%\com.chatvault.desktop\
  ...
  logs\
    ...
    scheduled-YYYY-MM-DD.log 定时运行日志（UTC 日期，保留七天）
```
...
系统定时运行的 CLI 使用独立日志，按 UTC 日期追加并保留最近七天，记录程序与数据库路径、运行 ID、阶段进展、结果和错误。运行历史来自 SQLite，不依赖控制台输出；新文件和上传计数与“立即运行”使用同一摘要结构。数据库打开前的错误无法写入运行历史，但日志初始化成功后仍会记录到定时日志。
